...
function makeControlPanel() {
  ...
  styleTag.innerHTML = `
                ...
                #menuToggle { display: block; position: relative; top: 5px; left: 5px; z-index: 1; -webkit-user-select: none; user-select: none; }

                #menuToggle #hamburgerCheckbox {display: block; width: 40px; height: 32px; position: absolute; top: -7px; left: -5px; cursor: pointer; opacity: 0; /* hide this */ z-index: 2; /* and place it over the hamburger */ -webkit-touch-callout: none;}

                /* Just a quick hamburger */
                #menuToggle span { display: block; width: 33px; height: 4px;   margin-bottom: 5px; position: relative; background: #cdcdcd; border-radius: 3px; z-index: 1; transform-origin: 4px 0px;   transition: transform 0.5s cubic-bezier(0.77,0.2,0.05,1.0),
...
function clickNextButton(buttonType, selector, scrollable = true) {
  ...
  if (nextButtonToPress !== null) {
    /*If the button exists, we should check if we should press it. If it doesn't exist, we scroll the page to see if we can generate more.*/
    if (canButtonsBeCurrentlyPressed === true && recentButtonsPressed < maximumFriendRequestsSent) {
      /*Checks to see if we've already reached the limit on how many buttons to press and stops pressing if so.*/
...
(function main() {
  ...
  if (window.location.href.indexOf("messenger.com") == -1 || window.location.href.indexOf("/messages/") == -1) {
    /*Makes sure that it doesn't load the addButtons on the messenger windows, because indexOf returns -1 if it doesn't show up.*/
    if (document.readyState === "loading") {
      /*Occasionally, it'll try to load before the page has loaded the Document Object Model (DOM). This is an issue. This checks if it's loaded or not.*/
